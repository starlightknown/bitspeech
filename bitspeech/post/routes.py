...
from bitspeech.community import community_service
from bitspeech.post import post_blueprint, post_service
from bitspeech.post.forms import PostForm, UpdatePostForm
...
@post_blueprint.route(
    "/community/<string:name>/post/<string:title>/diamond", methods=["POST"]
)
@login_required
def diamond(name, title):
    """
    Route that handles downvoting a post as the current user.
    """
    post = post_service.get_post(title, name)
    if post:
        post_service.diamond(post.id, current_user.id)
